...
exports.startTrendingService = startTrendingService;
exports.stopTrendingService = stopTrendingService;
...
async function computeScores() {
    ...
    // 1. Get all markets
    const mkts = await pool.query("SELECT id, spot_price, volume_usid, swap_count, holder_count, last_swap_at FROM market");
    if (mkts.rows.length === 0)
        return;
    // 2. Pre-fetch aggregate swap data for all markets in recent + previous windows
    const recentSwaps = await pool.query(`SELECT market_id,
            COUNT(*)::int AS cnt,
            COUNT(DISTINCT user_id)::int AS unique_traders,
            COALESCE(SUM(amount_in_usid::numeric), 0) AS vol
     FROM swap WHERE "timestamp" >= $1
     GROUP BY market_id`, [recentStart]);
    const prevSwaps = await pool.query(`SELECT market_id,
            COUNT(*)::int AS cnt,
            COALESCE(SUM(amount_in_usid::numeric), 0) AS vol
     FROM swap WHERE "timestamp" >= $1 AND "timestamp" < $2
     GROUP BY market_id`, [prevStart, recentStart]);
    // 3. Pre-fetch price an hour ago per market (from candles or swaps)
    const priceAnHourAgo = await pool.query(`SELECT DISTINCT ON (market_id) market_id, spot_price
     FROM swap
     WHERE "timestamp" <= $1
     ORDER BY market_id, "timestamp" DESC`, [recentStart]);
    // Build lookup maps
    ...
    const finals = rawScores.map((raw, i) => {
        const score = W.volumeVelocity * nVolVel[i] +
    ...
    try {
        ...
        for (let i = 0; i < finals.length; i++) {
            const f = finals[i];
            await client.query(`INSERT INTO trending_score
         ...
         ON CONFLICT (market_id) DO UPDATE SET
           ...
           freshness = $9, category = $10, updated_at = $11`, [f.marketId, f.score, i + 1, f.volumeVelocity, f.priceMomentum,
...
// ─── Category assignment ─────────────────────────────────────
function classifyRank(rank, total, score) {
...
// ─── Public API ──────────────────────────────────────────────
async function startTrendingService() {
    if (running)
        return;
    running = true;
...
async function stopTrendingService() {
    running = false;
    if (loopHandle)
