...
exports.handleTokenTransfer = handleTokenTransfer;
const db_1 = require("../db");
const helpers_1 = require("./helpers");
async function handleTokenTransfer(log, blockNumber, timestamp, txHash, logIndex, tokenAddress) {
    ...
    if (toAddress !== helpers_1.ZERO_ADDRESS) {
        ...
        if (tbRes.rows.length === 0) {
         ...
         VALUES ($1,$2,$3,$4,$5) ON CONFLICT (id) DO NOTHING`, [toBalanceId, toAddress, addr, amount, timestamp]);
            // New holder
            await db.query("UPDATE token SET holder_count = holder_count + 1 WHERE id = $1", [addr]);
        }
        else {
            const wasZero = (0, helpers_1.isZeroDec)(tbRes.rows[0].balance);
