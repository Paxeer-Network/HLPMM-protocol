...
import { getPool, getLastIndexedBlock, setLastIndexedBlock } from "./db";
import { config } from "./config";
import {
  EventEmitterABI, HLPMMFactoryABI, MarketNFTABI,
  HLPMMPoolABI, ERC20ABI,
} from "./abis";
import { handleMarketCreated, handleSwap, handleFeeClaimed, handleFeeStrategyUpdated } from "./handlers/event-emitter";
import { handleFactoryMarketCreated } from "./handlers/factory";
import { handleNFTTransfer, handleNFTFeeStrategyUpdated, handleNFTFeesClaimed } from "./handlers/market-nft";
import { handleSync } from "./handlers/pool";
import { handleTokenTransfer } from "./handlers/token";
import { handleUSIDTransfer } from "./handlers/usid";

// ─── Interfaces for parsed contract events ───────────────────
const eventEmitterIface = new ethers.Interface(EventEmitterABI);
const factoryIface = new ethers.Interface(HLPMMFactoryABI);
const marketNFTIface = new ethers.Interface(MarketNFTABI);
const poolIface = new ethers.Interface(HLPMMPoolABI);
...
async function getTrackedAddresses(): Promise<{ pools: Set<string>; tokens: Set<string> }> {
  const db = getPool();
  ...
  return {
    pools: new Set(poolsRes.rows.map((r: any) => r.address.toLowerCase())),
    tokens: new Set(tokensRes.rows.map((r: any) => r.address.toLowerCase())),
  };
}
...
async function getBlockTimestamp(blockNum: number): Promise<number> {
  ...
  if (blockTimestampCache.size > 10000) {
    ...
    if (first !== undefined) blockTimestampCache.delete(first);
  }
  return ts;
}

/** Process a single log entry, dispatching to the correct handler */
async function processLog(log: ethers.Log, timestamp: number, tracked: { pools: Set<string>; tokens: Set<string> }) {
  ...
  try {
    // ── EventEmitter ──
    if (addr === config.contracts.eventEmitter.toLowerCase()) {
      if (topic0 === TOPICS.emitterMarketCreated) {
        const parsed = eventEmitterIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleMarketCreated(parsed, blockNumber, timestamp, txHash, logIndex);
      } else if (topic0 === TOPICS.emitterSwap) {
        const parsed = eventEmitterIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleSwap(parsed, blockNumber, timestamp, txHash, logIndex);
      } else if (topic0 === TOPICS.emitterFeeClaimed) {
        const parsed = eventEmitterIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleFeeClaimed(parsed, blockNumber, timestamp, txHash, logIndex);
      } else if (topic0 === TOPICS.emitterFeeStrategyUpdated) {
        const parsed = eventEmitterIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleFeeStrategyUpdated(parsed, blockNumber, timestamp, txHash, logIndex);
      }
      return;
    }

    // ── Factory ──
    if (addr === config.contracts.factory.toLowerCase()) {
      if (topic0 === TOPICS.factoryMarketCreated) {
        const parsed = factoryIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleFactoryMarketCreated(parsed, blockNumber, timestamp, txHash, logIndex);
      }
      return;
    }

    // ── MarketNFT ──
    if (addr === config.contracts.marketNFT.toLowerCase()) {
      if (topic0 === TOPICS.nftTransfer) {
        const parsed = marketNFTIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleNFTTransfer(parsed, blockNumber, timestamp, txHash, logIndex);
      } else if (topic0 === TOPICS.nftFeeStrategyUpdated) {
        const parsed = marketNFTIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleNFTFeeStrategyUpdated(parsed, blockNumber, timestamp, txHash, logIndex);
      } else if (topic0 === TOPICS.nftFeesClaimed) {
        const parsed = marketNFTIface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleNFTFeesClaimed(parsed, blockNumber, timestamp, txHash, logIndex);
      }
      return;
    }

    // ── USID token ──
    if (addr === config.contracts.usid.toLowerCase()) {
      if (topic0 === TOPICS.erc20Transfer) {
        const parsed = erc20Iface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleUSIDTransfer(parsed, blockNumber, timestamp, txHash, logIndex);
      }
      return;
    }
    ...
    if (tracked.pools.has(addr)) {
      if (topic0 === TOPICS.poolSync) {
        ...
        if (parsed) await handleSync(parsed, blockNumber, timestamp, txHash, logIndex, addr);
      }
      return;
    }
    ...
    if (tracked.tokens.has(addr)) {
      if (topic0 === TOPICS.erc20Transfer) {
        const parsed = erc20Iface.parseLog({ topics: [...log.topics], data: log.data });
        if (parsed) await handleTokenTransfer(parsed, blockNumber, timestamp, txHash, logIndex, addr);
      }
      return;
    }
  } catch (err) {
...
function buildAddressList(tracked: { pools: Set<string>; tokens: Set<string> }): string[] {
  return [
    config.contracts.eventEmitter.toLowerCase(),
    config.contracts.factory.toLowerCase(),
    config.contracts.marketNFT.toLowerCase(),
    config.contracts.usid.toLowerCase(),
    ...tracked.pools,
    ...tracked.tokens,
  ];
}
...
async function fetchLogsRange(from: number, to: number, addresses: string[]): Promise<ethers.Log[]> {
  const p = getProvider();
  try {
...
export async function startIndexer(): Promise<void> {
  ...
  console.log(`[indexer] Batch size: ${config.batchSize} blocks, concurrency: ${config.concurrency}`);
  ...
  while (isRunning) {
    try {
      ...
      const totalRange = currentBlock - config.startBlock;
      const done = fromBlock - config.startBlock;
