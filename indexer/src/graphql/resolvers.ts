import { getPool, getLastIndexedBlock } from "../db";
...
function orderCol(orderBy?: string): string {
  if (!orderBy) return "id";
  return ORDER_MAP[orderBy] || "id";
}

function orderDir(dir?: string): string {
  return dir === "desc" ? "DESC" : "ASC";
}

function pagination(first?: number, skip?: number): string {
...
function buildWhereClause(where: any, params: any[]): string {
  ...
  for (const [key, value] of Object.entries(where)) {
    ...
    let op = "=";
    let baseField = key;
    if (key.endsWith("_gte")) { op = ">="; baseField = key.slice(0, -4); }
    else if (key.endsWith("_lte")) { op = "<="; baseField = key.slice(0, -4); }
    else if (key.endsWith("_gt")) { op = ">"; baseField = key.slice(0, -3); }
    else if (key.endsWith("_lt")) { op = "<"; baseField = key.slice(0, -3); }
...
function mapFeeStrategyChange(r: any) {
  if (!r) return null;
  return {
...
function mapOHLC(r: any, timeField: string) {
  if (!r) return null;
  return {
...
function mapMarketHourData(r: any) {
  if (!r) return null;
  return {
...
function mapMarketDayData(r: any) {
  if (!r) return null;
  return {
...
function mapMarket5MinData(r: any) {
  if (!r) return null;
  return {
...
function mapProtocolDayData(r: any) {
  if (!r) return null;
  return {
...
function mapUserTokenBalance(r: any) {
  if (!r) return null;
  return {
...
export const resolvers = {
  Query: {
    protocol: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM protocol WHERE id = $1", [id]);
      return mapProtocol(res.rows[0]);
    },
    protocols: async (_: any, { first, skip }: any) => {
      const res = await getPool().query(`SELECT * FROM protocol ${pagination(first, skip)}`);
      return res.rows.map(mapProtocol);
    },

    market: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM market WHERE id = $1", [id]);
      return mapMarket(res.rows[0]);
    },
    markets: async (_: any, { first, skip, orderBy, orderDirection, where }: any) => {
      const params: any[] = [];
    ...
    token: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM token WHERE id = $1", [id]);
      return mapToken(res.rows[0]);
    },
    tokens: async (_: any, { first, skip }: any) => {
      const res = await getPool().query(`SELECT * FROM token ORDER BY created_at DESC ${pagination(first, skip)}`);
      return res.rows.map(mapToken);
    },

    marketNFT: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM market_nft WHERE id = $1", [id]);
      return mapMarketNFT(res.rows[0]);
    },
    marketNFTs: async (_: any, { first, skip }: any) => {
      const res = await getPool().query(`SELECT * FROM market_nft ORDER BY minted_at DESC ${pagination(first, skip)}`);
      return res.rows.map(mapMarketNFT);
    },

    user: async (_: any, { id }: any) => {
      const res = await getPool().query('SELECT * FROM "user" WHERE id = $1', [id.toLowerCase()]);
      return mapUser(res.rows[0]);
    },
    users: async (_: any, { first, skip, orderBy, orderDirection }: any) => {
      const res = await getPool().query(`SELECT * FROM "user" ORDER BY ${orderCol(orderBy)} ${orderDir(orderDirection)} ${pagination(first, skip)}`);
      return res.rows.map(mapUser);
    },

    swap: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM swap WHERE id = $1", [id]);
      return mapSwap(res.rows[0]);
    },
    swaps: async (_: any, { first, skip, orderBy, orderDirection, where }: any) => {
      const params: any[] = [];
      const whereClause = buildWhereClause(where, params);
    ...
    feeClaim: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM fee_claim WHERE id = $1", [id]);
      return mapFeeClaim(res.rows[0]);
    },
    feeClaims: async (_: any, { first, skip }: any) => {
      const res = await getPool().query(`SELECT * FROM fee_claim ORDER BY "timestamp" DESC ${pagination(first, skip)}`);
      return res.rows.map(mapFeeClaim);
    },

    nftTransfer: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM nft_transfer WHERE id = $1", [id]);
      return mapNFTTransfer(res.rows[0]);
    },
    nftTransfers: async (_: any, { first, skip }: any) => {
      const res = await getPool().query(`SELECT * FROM nft_transfer ORDER BY "timestamp" DESC ${pagination(first, skip)}`);
      return res.rows.map(mapNFTTransfer);
    },

    feeStrategyChange: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM fee_strategy_change WHERE id = $1", [id]);
      return mapFeeStrategyChange(res.rows[0]);
    },
    feeStrategyChanges: async (_: any, { first, skip }: any) => {
      const res = await getPool().query(`SELECT * FROM fee_strategy_change ORDER BY "timestamp" DESC ${pagination(first, skip)}`);
      return res.rows.map(mapFeeStrategyChange);
    },

    marketHourDatas: async (_: any, { first, skip, where, orderBy, orderDirection }: any) => {
      const params: any[] = [];
      const whereClause = buildWhereClause(where, params);
    ...
    marketDayDatas: async (_: any, { first, skip, where, orderBy, orderDirection }: any) => {
      const params: any[] = [];
      const whereClause = buildWhereClause(where, params);
    ...
    market5MinDatas: async (_: any, { first, skip, where, orderBy, orderDirection }: any) => {
      const params: any[] = [];
      const whereClause = buildWhereClause(where, params);
    ...
    protocolDayDatas: async (_: any, { first, skip, orderBy, orderDirection }: any) => {
    ...
    userTokenBalance: async (_: any, { id }: any) => {
      const res = await getPool().query("SELECT * FROM user_token_balance WHERE id = $1", [id]);
      return mapUserTokenBalance(res.rows[0]);
    },
    userTokenBalances: async (_: any, { first, skip, where, orderBy, orderDirection }: any) => {
      const params: any[] = [];
      const whereClause = buildWhereClause(where, params);
    ...
    transaction: async (_: any, { id }: any) => {
      const res = await getPool().query('SELECT * FROM "transaction" WHERE id = $1', [id]);
      const r = res.rows[0];
      if (!r) return null;
      return { id: r.id, blockNumber: r.block_number, timestamp: r.timestamp, gasUsed: r.gas_used, gasPrice: r.gas_price };
    },

    candles: async (_: any, { market, timeframe, first, skip, orderBy, orderDirection, where }: any) => {
      const params: any[] = [market.toLowerCase(), timeframe];
      let conditions = "market_id = $1 AND timeframe = $2";

      if (where) {
        if (where.periodStart_gt != null) { params.push(where.periodStart_gt); conditions += ` AND period_start > $${params.length}`; }
        if (where.periodStart_lt != null) { params.push(where.periodStart_lt); conditions += ` AND period_start < $${params.length}`; }
        if (where.periodStart_gte != null) { params.push(where.periodStart_gte); conditions += ` AND period_start >= $${params.length}`; }
        if (where.periodStart_lte != null) { params.push(where.periodStart_lte); conditions += ` AND period_start <= $${params.length}`; }
      }
    ...
    trending: async (_: any, { first, skip, category }: any) => {
      const params: any[] = [];
      let where = "";
      if (category) {
    ...
    searchMarkets: async (_: any, { query, first }: any) => {
      const limit = Math.min(first ?? 20, 100);
      const res = await getPool().query(
        "SELECT * FROM market WHERE name ILIKE $1 OR symbol ILIKE $1 ORDER BY volume_usid::numeric DESC LIMIT $2",
        [`%${query}%`, limit]
      );
      return res.rows.map(mapMarket);
    },
  ...
  Market: {
    token: async (parent: any) => {
      const res = await getPool().query("SELECT * FROM token WHERE id = $1", [parent._tokenId]);
      return mapToken(res.rows[0]);
    },
    nft: async (parent: any) => {
      const res = await getPool().query("SELECT * FROM market_nft WHERE id = $1", [parent._nftId]);
      return mapMarketNFT(res.rows[0]);
    },
    creator: async (parent: any) => {
      const res = await getPool().query('SELECT * FROM "user" WHERE id = $1', [parent._creatorId]);
      return mapUser(res.rows[0]);
    },
    swaps: async (parent: any, { first, skip, orderBy, orderDirection }: any) => {
      const res = await getPool().query(
        `SELECT * FROM swap WHERE market_id = $1 ORDER BY ${orderCol(orderBy || "timestamp")} ${orderDir(orderDirection || "desc")} ${pagination(first, skip)}`,
        [parent.id]
      );
      return res.rows.map(mapSwap);
    },
    hourData: async (parent: any, { first, skip, orderBy, orderDirection }: any) => {
      const res = await getPool().query(
        `SELECT * FROM market_hour_data WHERE market_id = $1 ORDER BY ${orderCol(orderBy || "hourStartUnix")} ${orderDir(orderDirection || "desc")} ${pagination(first, skip)}`,
        [parent.id]
      );
      return res.rows.map(mapMarketHourData);
    },
    dayData: async (parent: any, { first, skip, orderBy, orderDirection }: any) => {
      const res = await getPool().query(
        `SELECT * FROM market_day_data WHERE market_id = $1 ORDER BY ${orderCol(orderBy || "dayStartUnix")} ${orderDir(orderDirection || "desc")} ${pagination(first, skip)}`,
        [parent.id]
      );
      return res.rows.map(mapMarketDayData);
    },
    fiveMinData: async (parent: any, { first, skip, orderBy, orderDirection }: any) => {
      const res = await getPool().query(
        `SELECT * FROM market_5min_data WHERE market_id = $1 ORDER BY ${orderCol(orderBy || "periodStart")} ${orderDir(orderDirection || "desc")} ${pagination(first, skip)}`,
        [parent.id]
      );
      return res.rows.map(mapMarket5MinData);
    },
  ...
  Token: {
    market: async (parent: any) => {
      if (!parent._marketId) return null;
      const res = await getPool().query("SELECT * FROM market WHERE id = $1", [parent._marketId]);
      return mapMarket(res.rows[0]);
    },
  },
  ...
  MarketNFT: {
    owner: async (parent: any) => {
      const res = await getPool().query('SELECT * FROM "user" WHERE id = $1', [parent._ownerId]);
      return mapUser(res.rows[0]);
    },
    market: async (parent: any) => {
      const res = await getPool().query("SELECT * FROM market WHERE id = $1", [parent._marketId]);
    ...
    claims: async (parent: any, { first, skip }: any) => {
      const res = await getPool().query(
        `SELECT * FROM fee_claim WHERE nft_id = $1 ORDER BY "timestamp" DESC ${pagination(first, skip)}`, [parent.id]
      );
      return res.rows.map(mapFeeClaim);
    },
    transfers: async (parent: any, { first, skip }: any) => {
      const res = await getPool().query(
        `SELECT * FROM nft_transfer WHERE nft_id = $1 ORDER BY "timestamp" DESC ${pagination(first, skip)}`, [parent.id]
      );
      return res.rows.map(mapNFTTransfer);
    },
  },
  ...
  User: {
    swaps: async (parent: any, { first, skip, orderBy, orderDirection }: any) => {
      const res = await getPool().query(
        `SELECT * FROM swap WHERE user_id = $1 ORDER BY ${orderCol(orderBy || "timestamp")} ${orderDir(orderDirection || "desc")} ${pagination(first, skip)}`,
        [parent.id]
      );
      return res.rows.map(mapSwap);
    },
    ownedNFTs: async (parent: any, { first, skip }: any) => {
      const res = await getPool().query(
        `SELECT * FROM market_nft WHERE owner_id = $1 ${pagination(first, skip)}`, [parent.id]
      );
      return res.rows.map(mapMarketNFT);
    },
    createdMarkets: async (parent: any, { first, skip }: any) => {
      const res = await getPool().query(
        `SELECT * FROM market WHERE creator_id = $1 ORDER BY created_at DESC ${pagination(first, skip)}`, [parent.id]
      );
      return res.rows.map(mapMarket);
    },
    tokenBalances: async (parent: any, { first, skip }: any) => {
      const res = await getPool().query(
        `SELECT * FROM user_token_balance WHERE user_id = $1 ${pagination(first, skip)}`, [parent.id]
      );
      return res.rows.map(mapUserTokenBalance);
    },
  },
  ...
  Swap: {
    ...
    user: async (parent: any) => {
      const res = await getPool().query('SELECT * FROM "user" WHERE id = $1', [parent._userId]);
      return mapUser(res.rows[0]);
    },
  },
  ...
  FeeClaim: {
    ...
    recipient: async (parent: any) => {
      const res = await getPool().query('SELECT * FROM "user" WHERE id = $1', [parent._recipientId]);
      return mapUser(res.rows[0]);
    },
  },
  ...
  NFTTransfer: {
    ...
    from: async (parent: any) => {
      const res = await getPool().query('SELECT * FROM "user" WHERE id = $1', [parent._fromId]);
      return mapUser(res.rows[0]);
    },
    to: async (parent: any) => {
      const res = await getPool().query('SELECT * FROM "user" WHERE id = $1', [parent._toId]);
      return mapUser(res.rows[0]);
    },
  },
